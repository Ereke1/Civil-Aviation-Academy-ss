...
function showNotFor11kl() {
    document.getElementById('programms').value = "";
    // Скрываем недоступные обр. программы
    let notFor11kl = document.querySelectorAll('.not-for-11kl')
    for (let i = 0; i < notFor11kl.length; i++) {
        notFor11kl[i].classList.remove('disabled')
    }
    let for11kl = document.querySelectorAll('.for-11kl')
    for (let i = 0; i < for11kl.length; i++) {
        for11kl[i].classList.add('disabled')
    }
}

function showFor11kl() {
    document.getElementById('programms').value = "";
    // Скрываем недоступные обр. программы
    let notFor11kl = document.querySelectorAll('.not-for-11kl')
    for (let i = 0; i < notFor11kl.length; i++) {
        notFor11kl[i].classList.add('disabled')
    }
    let for11kl = document.querySelectorAll('.for-11kl')
    for (let i = 0; i < for11kl.length; i++) {
        for11kl[i].classList.remove('disabled')
    }
}
...
function haveENTFunc() {
    ...
    if (admissionInputVal() === '11-го класса' && haveENTInputVal() === 'Да') {
        ...
        showNotFor11kl();
    } else if (admissionInputVal() === '11-го класса' && haveENTInputVal() === 'Нет') {
        document.getElementById('mathLitBlock').classList.add('disabled')
        document.getElementById('mathLit').value = "";
        document.getElementById('mathLit').required = false
        document.getElementById('readLitBlock').classList.add('disabled')
        ...
        document.getElementById('profSubBlock').classList.add('disabled')
        document.getElementById('profSub').value = "";
        document.getElementById('profSub').required = false
